...
from rag.retrieval import rerank_documents, retrieve_documents, retrieve_documents_with_scores, rewrite_query
...
def _mock_rerank(documents, top_n: int):
	...
	for rank, document in enumerate(documents[:top_n], start=1):
		...
		mocked_documents.append(Document(page_content=document.document.page_content, metadata=metadata))
