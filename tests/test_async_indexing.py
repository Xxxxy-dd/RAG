...
def test_enqueue_index_from_file(monkeypatch) -> None:
	chunks = [
		Document(page_content="三层CNN用于验证码识别。", metadata={"source": "demo.docx", "title_path": "方法 > 模型结构", "section_id": 2, "chunk_id": 1}),
		Document(page_content="池化层用于下采样。", metadata={"source": "demo.docx", "title_path": "方法 > 模型结构", "section_id": 2, "chunk_id": 2}),
	]
	calls = []

	monkeypatch.setattr(index_manager, "ingest_file", lambda path, chunking_config=None: chunks)
	monkeypatch.setattr(index_manager, "append_index_chunk_event", lambda **kwargs: calls.append(kwargs) or f"evt-{len(calls)}")
...
def test_vector_worker_process_once(monkeypatch) -> None:
	...
	monkeypatch.setattr(vector_worker, "ack_index_chunk_event", lambda event_id, **kwargs: calls.append(("ack", event_id)) or 1)
