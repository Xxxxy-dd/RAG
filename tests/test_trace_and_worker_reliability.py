...
def test_api_request_propagates_trace_id_header(monkeypatch) -> None:
	...
	response = client.post("/api/qa", headers={"x-trace-id": "trace-123"}, json={"question": "测试问题", "session_id": "sess-1"})
...
def test_enqueue_index_from_file_propagates_trace_id(monkeypatch) -> None:
	chunks = [
		Document(page_content="三层CNN用于验证码识别。", metadata={"source": "demo.docx", "title_path": "方法 > 模型结构"}),
	]
	captured = []

	monkeypatch.setattr(index_manager, "ingest_file", lambda path, chunking_config=None: chunks)
	monkeypatch.setattr(index_manager, "append_index_chunk_event", lambda **kwargs: captured.append(kwargs) or "evt-1")
...
def test_vector_worker_success_persists_trace_id(monkeypatch) -> None:
	...
	monkeypatch.setattr(vector_worker, "ack_index_chunk_event", lambda event_id, **kwargs: acked.append(str(event_id)) or 1)
	...
	worker = vector_worker.VectorWorker(collection_name="document_indexing", persist_directory="E:/tmp/chroma", max_retries=3)
...
def test_vector_worker_duplicate_event_is_skipped(monkeypatch) -> None:
	...
	worker = vector_worker.VectorWorker(collection_name="document_indexing", persist_directory="E:/tmp/chroma")
	count = worker.process_once()
...
def test_vector_worker_deadletters_after_max_retries(monkeypatch) -> None:
	...
	monkeypatch.setattr(vector_worker, "send_to_dead_letter", lambda **kwargs: dlq_calls.append(kwargs) or "dlq-1")
	monkeypatch.setattr(vector_worker, "clear_retry", lambda *args, **kwargs: None)
	monkeypatch.setattr(vector_worker.time, "sleep", lambda *args, **kwargs: None)

	worker = vector_worker.VectorWorker(collection_name="document_indexing", persist_directory="E:/tmp/chroma", max_retries=1)
	count = worker.process_once()
...
def test_persist_worker_deadletters_after_max_retries(monkeypatch) -> None:
	...
	monkeypatch.setattr(persist_worker, "ack_qa_turn_event", lambda event_id, **kwargs: acked.append(str(event_id)) or 1)
	...
	monkeypatch.setattr(persist_worker, "send_to_dead_letter", lambda **kwargs: dlq_calls.append(kwargs) or "dlq-1")
