...
def _require_driver() -> None:
	if pymysql is None or DictCursor is None:
		raise RuntimeError("pymysql package is not installed; install requirements before using MySQL persistence")
...
class MySQLStore:
	...
	def ensure_schema(self) -> None:
		...
		if sa_metadata is not None:
			...
			with self.connection() as conn:
				...
				engine = create_engine(f"mysql+pymysql://{self.settings.user}:{self.settings.password}@{self.settings.host}:{self.settings.port}/{self.settings.database}?charset={self.settings.charset}")
	...
	def ensure_conversation(self, session_id: str, user_id: str | None = None, title: str | None = None) -> int:
		with self.connection() as conn:
			with conn.cursor() as cursor:
				cursor.execute(
					"""
	...
	def _next_sequence(self, cursor, conversation_id: int) -> int:
		cursor.execute("SELECT COALESCE(MAX(seq), 0) AS max_seq FROM messages WHERE conversation_id = %s", (conversation_id,))
	...
	def append_message(
		...
		*,
		...
		try:
			conversation_id = self.ensure_conversation(session_id=session_id, user_id=user_id, title=title)
			message_id = message_id or uuid.uuid4().hex
			metadata_text = json.dumps(metadata, ensure_ascii=False) if metadata is not None else None
				...
				with conn.cursor() as cursor:
					...
					cursor.execute(
						...
						ON DUPLICATE KEY UPDATE
							...
							updated_at = CURRENT_TIMESTAMP
						""",
						(conversation_id, message_id, trace_id, idempotency_key, role, text, seq, metadata_text),
					)
					cursor.execute(
			...
			LOGGER.exception("MySQL append_message failed: session_id=%s role=%s message_id=%s", session_id, role, message_id)
			raise
	...
	idempotency_key: str | None = None,
		...
		try:
			conversation_id = self.ensure_conversation(session_id=session_id, user_id=user_id, title=conversation_title)
				...
				with conn.cursor() as cursor:
					cursor.execute(
						...
						ON DUPLICATE KEY UPDATE
							...
							updated_at = CURRENT_TIMESTAMP
						""",
						(document_key, trace_id, idempotency_key, source, title_path, content, content_hash, metadata_text),
					)
					cursor.execute(
	...
	idempotency_key: str | None = None,
		...
		try:
			...
			resolved_dimension = dimension if dimension is not None else resolve_embedding_dimension(model)
			with self.connection() as conn:
				with conn.cursor() as cursor:
					cursor.execute(
						"""
						...
						ON DUPLICATE KEY UPDATE
							...
							updated_at = CURRENT_TIMESTAMP
						""",
						(document_id, vector_id, trace_id, idempotency_key, resolved_model, resolved_dimension, metadata_text),
					)
					cursor.execute(
			...
			LOGGER.exception("MySQL save_embedding failed: vector_id=%s document_id=%s", vector_id, document_id)
			raise
...
def get_mysql_store() -> MySQLStore | None:
	...
	if settings is None:
		return None
	if pymysql is None or DictCursor is None:
		LOGGER.warning("MySQL settings are configured, but pymysql is not installed; skipping persistence")
		return None
	...
	*,
	...
	for chunk in chunks:
		...
		document_row = store.save_document(
			...
			title_path=str(metadata.get("title_path")) if metadata.get("title_path") is not None else None,
			trace_id=str(metadata.get("trace_id")) if metadata.get("trace_id") is not None else None,
			idempotency_key=str(metadata.get("idempotency_key")) if metadata.get("idempotency_key") is not None else None,
