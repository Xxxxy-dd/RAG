...
class RerankerService:
	...
	def rerank(
		...
		for rank, (score, document, original_rank, original_score) in enumerate(scored_items[:limit], start=1):
	...
	def rerank_documents(
		...
		"""对候选文档重排并仅返回文档列表。"""
		return [item.document for item in self.rerank(query=query, documents=documents, top_n=top_n)]
