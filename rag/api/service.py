...
from ..retrieval import RetrievedDocument, RerankedDocument, rerank_with_details, retrieve_documents_with_scores, rewrite_query
from ..memory.redis_memory import append_qa_turn_event, get_recent_messages, save_message, save_retrieval_snapshot
...
def _build_answer_prompt(question: str, rewritten_question: str, chunks: Sequence[ContextChunk]) -> str:
	...
	*,
	...
	if session_id:
		try:
			...
			LOGGER.warning("Failed to load recent messages for session_id=%s: %s", session_id, exc, exc_info=True)
			merged_history = []

	if history:
		merged_history.extend(list(history))

	rewritten_question = rewrite_query(raw_question, history=merged_history) if use_query_rewrite else raw_question
	LOGGER.info(
	...
	if reranked:
		context_chunks = [_to_context_chunk_from_reranked(index=i, item=item) for i, item in enumerate(reranked, start=1)]
	elif retrieved:
		context_chunks = [_to_context_chunk_from_retrieved(index=i, item=item) for i, item in enumerate(retrieved[:top_n], start=1)]

	prompt = _build_answer_prompt(raw_question, rewritten_question, context_chunks)
	client = chat_client or ChatClient()
	answer = client.complete(prompt=prompt, system_prompt="你是严谨的知识库问答助手，请基于上下文回答。")
	...
	if session_id:
		try:
			...
			append_qa_turn_event(
				...
				user_metadata={"source": "api", "kind": "question", "turn_id": turn_id, "trace_id": active_trace_id},
				assistant_metadata={"source": "api", "kind": "answer", "turn_id": turn_id, "trace_id": active_trace_id},
				snapshot=snapshot,
			)
		except Exception:
			# 持久化失败不阻断主回答返回，但日志必须可见。
			LOGGER.exception("Failed to persist QA turn for session_id=%s", session_id, extra={"event": "qa_turn_enqueue_failed", "trace_id": active_trace_id})
